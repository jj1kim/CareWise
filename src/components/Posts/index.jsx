...
export const DetailedPost = ({ post }) => {
  ...
  const onSubmit = (e) => {
    ...
    if (replyFor) {
      ...
      const filteredComments = comments.filter((comment) => comment !== replyFor);
      const newComment = {...replyFor, replies: [...replyFor.replies, createdReply]}
...
<div className="bg-white w-[1043px] flex-col justify-center">
      ...
      <div className="flex justify-between mt-[31px] mb-[33px]">
        <button onClick={() => {navigate("/qna-board")}} className="flex w-[154px] h-[33px] items-center justify-center rounded-[15px] border border-solid border-[#a4a3a3]">
          <div className="[font-family:'Inter-Regular',Helvetica] font-normal text-[#757575] text-[13px] tracking-[0] leading-[normal]">
            목록으로 돌아가기
          </div>
        </button>

        <button onClick={() => {navigate("/post-create")}} className="w-[136px] h-[33px] gap-1 flex items-center justify-center rounded-[15px] border border-solid border-[#a4a3a3]">
          <img
            className="w-5 h-5"
            alt="Pencil"
            src={Pencil}
      ...
      <div className="border-[0.5px] border-solid border-[#a4a3a3] rounded-[10px] mb-[29px] px-[66px] py-[56px]">
        ...
        <div>
          {comments.map((comment) => (
            <div>        
            <div className={comment === replyFor? "px-[20px] py-[25px] rounded-[10px] bg-[#F6F6F6]" : "px-[20px] py-[25px]"}>
              <div className="flex justify-between">
                ...
                <div className="flex items-center gap-[11px]">
                  ...
                  <button onClick={() => {
                    if (replyFor === comment) {setReplyFor(0);} else {setReplyFor(comment);}
        ...
        <form onSubmit={onSubmit} className="mt-[30px] flex w-full items-center justify-between gap-[14px] px-[25px] py-[15px] bg-[#f3f3f3] rounded-[10px]">
          <img
            className="w-5 h-[18px]"
            alt="Speech Bubble"
            src={SpeechBubble}
            />
          <input id="content" onChange={handleChange} className="w-full [font-family:'Inter-Regular',Helvetica] font-normal bg-[#f3f3f3] text-[#757575] text-[17px] tracking-[0] leading-[normal]" placeholder="댓글을 입력하세요" required>
          </input>
    ...
    <Link to={`/post/${post.id}`} className="w-full flex-col">
      ...
      <div className="flex justify-between mb-[26px]">
        ...
        <div className="flex gap-3">
          ...
          <div className="flex items-center gap-[7px]">
            <img className="w-[13px] h-3" alt="SpeechBubble" src={SpeechBubble} />
            <div className="[font-family:'Inter-Regular',Helvetica] font-normal text-[#555555] text-[13px] tracking-[0] leading-[normal]">
